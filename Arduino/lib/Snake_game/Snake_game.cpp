...
Snake::Snake(uint8_t din, uint8_t cs, uint8_t clk) : matrix(din, cs, clk){};

void Snake::init(uint16_t screen_refresh_period_arg, uint8_t LED_intensity){
    matrix.init();
    matrix.sendData(max7219_reg_intensity, LED_intensity); // configure intensity
    Serial.begin(115200);
    randomSeed(analogRead(0)); // use analogRead 0'th port to set the seed for random number generator
...
void Snake::place_fruit(){
    while(fruit_exists == 0){
        fruit_exists = 1;
        x_coord_fruit = random(8); // generate random number with a value ranging from 0 to 7
        y_coord_fruit = random(8);
        for(int i=0;i<snake_len;i++){
...
uint8_t Snake::receive_movement_command(){
    int serial_data_in_len = Serial.available(); // return bytes waiting to be read
	if (serial_data_in_len > 0){
        char serial_data_in;
        // flush the buffer and take only the most recent value for further processing if buffer > 5
        if (serial_data_in_len > 5){
            for(int i=0;i<serial_data_in_len;i++){
                serial_data_in = Serial.read();
            }
        }
        else serial_data_in = Serial.read();
